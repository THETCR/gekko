...
Trader.prototype.getRawOpenOrders = function (callback) {
  const fetch = next => this.poloniex.returnOpenOrders(this.currency,
    this.asset,
...
Trader.prototype.createOrder = function (side, amount, price, callback) {
  ...
  const fetch = next => {
    this.poloniex[side](this.currency,
      this.asset,
      price,
      amount,
      this.processResponse(next, 'order', side))
  };
  retry(null, fetch, handle);
};
...
Trader.prototype.checkOrder = function (id, callback) {
  const handle = (err, result) => {
    ...
    const order = _.find(result, function (o) {
    ...
    callback(undefined,
      { executed: false, open: true, filledAmount: order.startingAmount - order.amount });
  };

  const fetch = next => this.poloniex.myOpenOrders(this.currency,
    this.asset,
    this.processResponse(next, 'checkOrder'));
  retry(null, fetch, handle);
};
...
Trader.prototype.getOrder = function (order, callback) {
  ...
  const fetch = next => this.poloniex.returnOrderTrades(order,
    this.processResponse(next, 'getOrder', order));
  retry(null, fetch, handle);
};
...
Trader.prototype.cancelOrder = function (order, callback) {
  ...
  const fetch = next => this.poloniex.cancelOrder(this.currency,
    this.asset,
    order,
    this.processResponse(next, 'cancelOrder', order));
  retry(null, fetch, handle);
};
...
Trader.prototype.getTrades = function (since, callback, descending) {
  ...
  const fetch = next => this.poloniex._public('returnTradeHistory',
    params,
    this.processResponse(next, 'getTrades', since));
  retry(null, fetch, handle);
};
