...
Trader.prototype.getTicker = function (callback) {
  ...
  const fetch = next => this.bittrexApi.getticker({ market: this.pair },
...
Trader.prototype.buy = function (amount, price, callback) {
  ...
  const fetch = next => this.bittrexApi.buylimit({
    market: this.pair,
    quantity: amount,
    rate: price
  }, this.processResponse(next));
...
Trader.prototype.sell = function (amount, price, callback) {
  ...
  const fetch = next => this.bittrexApi.selllimit({
    market: this.pair,
    quantity: amount,
    rate: price
  }, this.processResponse(next));
...
Trader.prototype.checkOrder = function (order, callback) {
  ...
  const fetch = next => this.bittrexApi.getopenorders({ market: this.pair },
    this.processResponse(next));
  retry(null, fetch, handle);
};
...
Trader.prototype.getTrades = function (since, callback, descending) {
  ...
  const fetch = next => this.bittrexApi.getmarkethistory({ market: params.currencyPair },
    this.processResponse(next));
  retry(null, fetch, handle);
};
