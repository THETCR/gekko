...
const Trader = function (config) {
  ...
  this.bitfinex = new Bitfinex.RESTv1({
    apiKey: this.key,
...
Trader.prototype.getOrder = function (order_id, callback) {
  const processResponse = (err, data) => {
    ...
    const feeFetcher = cb => this.bitfinex.past_trades(this.currency,
      { since: data.timestamp },
      this.handleResponse('pastTrades', cb));
...
Trader.prototype.cancelOrder = function (order_id, callback) {
  ...
  const handler = cb => this.bitfinex.cancel_order(order_id,
